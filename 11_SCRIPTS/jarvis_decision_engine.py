...
def decide(goal: str) -> dict:
    ...
    if not status["clean"]:
        return {
            "action": "review",
            "reason": "git is dirty; review before continuing",
            "command": ["review"],
            "status": status,
            "next_task": next_task,
        }
    ...
    if next_task:
        return {
            "action": "task-next",
            "reason": "safe queued task exists",
            "command": ["task", "next"],
            "status": status,
            "next_task": next_task,
        }
    ...
    return {
        "action": "mission",
        "reason": "no queued tasks; run controlled mission",
        "command": ["mission", goal, "--steps", "1"],
        "status": status,
        "next_task": None,
    }
...
def execute(goal: str, plan_only: bool = False) -> int:
    ...
    lines = [
        "# JARVIS Decision Engine — Block 122",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="JARVIS Block 122 Decision Engine")
