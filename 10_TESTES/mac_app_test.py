    ...
    def test_bundle_has_launcher_plist_and_survives_reinstall(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch.object(MODULE, "install_root", lambda system_wide: home / "Applications"), \
                ...
                self.assertIn("isMovableByWindowBackground = false", source)
                self.assertIn("OverlayWindow", source)
                self.assertIn("OverlayWebView", source)
                self.assertIn("acceptsFirstMouse", source)
                self.assertIn("installDrag", source)
                self.assertIn("handleWindowDrag", source)
                self.assertIn("wakeOnly", source)
                self.assertIn('body == "sleep"', source)
                self.assertIn('body == "arm"', source)
                self.assertIn("addGlobalMonitorForEvents", source)
                ...
                self.assertIn("replySize", source)
                self.assertIn("replyChip", source)
                self.assertIn("holdReply", source)
                self.assertIn("compactFrameSize", source)
                self.assertIn("width: 72", source)
                self.assertIn("width: 220", source)
                self.assertIn("height: 84", source)
                self.assertIn("width: 268", source)
                self.assertIn("height: 380", source)
                self.assertIn("hasPrefix(\"reply:\")", source)
                self.assertIn("CGRequestScreenCaptureAccess", source)
                self.assertIn("isHidden = true", source)
                self.assertIn("JarvisParkedOrigin", source)
                self.assertIn("JarvisOwnerToken", source)
                self.assertIn("activeScreenFrame", source)
