...
class UIQualityTest(unittest.TestCase):
    ...
    def test_dialogs_have_valid_accessible_titles(self):
        ...
        self.assertEqual(len(dialogs), 8)
    ...
    def test_composer_uses_one_whatsapp_style_voice_or_send_action(self):
        ...
        self.assertIn("grid-template-columns: 40px 56px 42px 42px 42px minmax(0, 1fr)", self.shell_css)
    ...
    def test_api_vault_and_n8n_forge_are_real_controls(self):
        ...
        self.assertIn("JARVIS · 3×", self.app_js)
    ...
    def test_startup_assets_stay_within_budget(self):
        ...
        self.assertLess(FEATURE_LOADER_JS.stat().st_size, 8 * 1024)
        ...
        self.assertLess(SHELL_CSS.stat().st_size, 20 * 1024)
    ...
    def test_mission_control_is_lazy_real_and_operator_controlled(self):
        ...
        self.assertIn('id="codePadButton"', self.html)
        self.assertIn(">Code</span>", self.html)
        self.assertIn('id="codeLaunchDialog"', self.html)
        self.assertIn('id="downloadButton"', self.html)
        self.assertIn(">App</span>", self.html)
        self.assertIn('id="paletteButton"', self.html)
        self.assertIn('id="commandPalette"', self.html)
        self.assertIn("Baixar o app · Mac e Windows", self.html)
        self.assertIn('id="windowsDownloadButton"', self.html)
    ...
    def test_notes_pad_title_input_does_not_reuse_dialog_heading_id(self):
        ...
        self.assertIn("notesPadOpenMac", notes_pad)
        self.assertIn("abre as notas", notes_pad)

    def test_code_pad_is_lazy_and_does_not_reuse_dialog_heading_id(self):
        code_pad = (WEB / "code-pad.js").read_text(encoding="utf-8")
        heading = re.search(
            r'ensureDialog\("codePadDialog", "code-pad-dialog", "([^"]+)"',
            self.feature_loader_js,
        )
        self.assertIsNotNone(heading)
        self.assertEqual(heading.group(1), "codePadHeading")
        self.assertNotEqual(heading.group(1), "codePadFilename")
        self.assertIn('id="codePadFilename"', code_pad)
        self.assertIn('id="codePadAlter"', code_pad)
        self.assertIn('id="codePadTerminal"', code_pad)
        self.assertIn("Usar no terminal", code_pad)
        self.assertIn("vscode://theopadilha.jarvis-theo/from-clipboard", code_pad)
        self.assertIn("/download/vscode/mac", code_pad)
        self.assertIn("/download/vscode/windows", code_pad)
        self.assertIn("Colar no VS Code", code_pad)
        self.assertIn("Pedir ao JARVIS", code_pad)
        self.assertIn("data-system-tab", self.html)
        self.assertIn("downloadDialogTitle", self.html)
        self.assertIn("jarvis-open-code", self.feature_loader_js)
        self.assertIn("COMMAND_PALETTE", self.app_js)
        self.assertIn("paletteButton", self.app_js)
        self.assertIn('data.client_action === "open_code_pad"', self.app_js)
        self.assertIn('data.client_action === "copy_text"', self.app_js)
        self.assertIn("https://grok.x.ai", self.app_js)
        self.assertIn("abre a pasta downloads", self.app_js)
        self.assertIn("jarvis-open-code", self.app_js)
        self.assertIn("open-code-pad", self.app_js)
        self.assertIn("20260819-ui2", self.html)
    ...
    def test_living_voice_preserves_voice_and_prefetches_natural_lead(self):
        ...
        self.assertIn('const SHELL_VERSION = "20260821-edit1"', self.app_js)
        self.assertIn("restartForUpdate", self.app_js)
        self.assertIn('id="updateToast"', self.html)
        self.assertIn('id="updateReloadButton"', self.html)
        self.assertIn('id="shellVersion"', self.html)
    ...
    def test_3d_is_lazy_quality_controlled_and_fully_pauses(self):
        ...
        self.assertIn('import("/ui/jarvis-3d.js?v=20260819-aug15")', self.presence_loader_js)
        ...
        self.assertNotIn("makeVisitorAlbedo", self.presence_js)
    ...
    def test_all_shell_assets_share_space_cache_version(self):
        ...
        self.assertGreaterEqual(self.html.count("20260819-ui2"), 2)
        self.assertIn("20260819-ui2", self.html)
        self.assertIn("20260821-edit1", self.html)
    ...
    def test_ultron_mode_has_distinct_identity_environment_and_strength_control(self):
        ...
        self.assertIn("Sair do JARVIS dono", self.app_js)
        ...
        self.assertIn("function browserOpenFor", self.app_js)
        self.assertIn("open-link", self.app_js)
        self.assertIn("debug=1", self.app_js)
        self.assertIn("youtube.com/results", self.app_js)
        self.assertIn("function mountDebug", self.app_js)
        self.assertNotIn("localOpen && options.source !== \"voice\"", self.app_js)
        self.assertIn("www.instagram.com", self.app_js)
        ...
        self.assertIn('data-strength="maximum"', self.html)
        self.assertIn("FORÇA MÁXIMA", self.app_js)
        ...
        self.assertIn('document.documentElement.dataset.persona = "jarvis"', self.app_js)
        self.assertIn("strength: session.paired && session.strength === \"auto\" ? \"maximum\" : session.strength", self.app_js)
        self.assertIn('"jarvis-response-strength"', self.app_js)
        self.assertIn("jarvis-strength-owner-max-v1", self.app_js)
        self.assertIn('return RESPONSE_STRENGTH.includes(saved) ? saved : "maximum"', self.app_js)
        ...
        self.assertIn('id="githubStarButton"', self.html)
        self.assertIn("/github-star", self.feature_loader_js)
        self.assertIn("github-star-button", self.shell_css)
        self.assertIn("jarvisCrownGrad", self.html)
        self.assertNotIn("ghbtns.com", self.html)
        self.assertIn('data.persona?.id === "jarvis_owner"', self.app_js)
        ...
        self.assertIn("ultron-red-identity-v2-", self.presence_js)
    ...
    def test_chat_window_is_a_real_window_without_height_jump(self):
        ...
        self.assertIn('placeholder="Escreva, fale ou digite / para ações"', self.html)
    ...
    def test_voice_never_falls_back_to_the_flat_default(self):
        ...
        self.assertNotIn("ElevenLabs sem créditos", self.app_js)
        self.assertIn("markElevenlabsExhausted", self.app_js)
        self.assertIn("__use_terminal__", self.app_js)
        self.assertIn("detail.auto", (WEB / "code-pad.js").read_text(encoding="utf-8"))
