...
- `./jarvis self-edit "melhoria" [--dry-run]` — cria branch e worktree locais exclusivos, chama o Codex CLI para editar os próprios scripts, exige diff real e só cria commit local depois de `bash -n`, `diff --check`, `py_compile` aplicável, `command-audit` e `safety-gate`; nunca faz push, merge ou deploy.
