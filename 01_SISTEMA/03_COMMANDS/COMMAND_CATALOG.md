...
- `./jarvis self-edit "melhoria" [--dry-run]` — cria branch e worktree locais exclusivos, chama o Codex CLI para editar os próprios scripts, exige diff real, roda `bash -n`, `diff --check`, `py_compile` aplicável e `command-audit`, cria um checkpoint local e então exige `safety-gate` com árvore limpa; nunca faz push, merge ou deploy.
