...
- `./jarvis spotify status|play|pause|next|previous|volume N|shuffle on|off|repeat on|off|search TEXTO|play-uri URI [--dry-run]` — controla o app Spotify instalado via contrato AppleScript oficial do próprio aplicativo, confirma estado/faixa/volume após a ação e nunca transforma texto livre em shell; busca abre resultados sem fingir que uma faixa começou.
...
- `./jarvis self-edit "melhoria" [--dry-run] [--publish]` — cria branch e worktree exclusivos, chama o Codex CLI para editar os próprios scripts, exige diff real, roda `bash -n`, `diff --check`, `py_compile` aplicável, `command-audit` e `safety-gate`. Sem `--publish`, termina no checkpoint local. Com `--publish` explicitamente autorizado, valida os alvos fixos, envia somente ao `jarvis-origin`, abre e mescla PR em `theopadilha2009-hash/jarvis-agent-os/main`, faz deploy somente no projeto Vercel `jarvis-agent-os`, aponta `jarvis-theo.vercel.app` para esse deploy, verifica `/status`, ativa o commit localmente e reinicia o worker.
- `./jarvis promote-production [--dry-run] [--deployment URL]` — aponta `jarvis-theo.vercel.app` para o último deploy de produção do projeto Vercel `jarvis-agent-os` (ou para `--deployment`). `JARVIS_NO_REPORT=1` força preview. Nunca escolhe outro projeto ou domínio.
- `./jarvis files-triage [PASTA] [--limit N]` — classifica arquivos soltos por extensão e mostra origem → destino; não possui `--apply`.
- `./jarvis battery [--dry-run]` — telemetria da bateria do Mac (percentual, fonte AC/Bateria, tempo restante, ciclo de carga, condição e capacidade máxima) via `pmset` e `system_profiler`.
- `./jarvis system-volume [status|mute|unmute|0-100] [--dry-run]` — consulta ou ajusta o volume master do macOS via contrato AppleScript seguro, com suporte a silenciar/reativar som.
- `./jarvis wifi-info [--dry-run]` — telemetria de rede Wi-Fi local do Mac (SSID conectado, IP local, gateway, canal/frequência, força do sinal RSSI, ruído, modo PHY e taxa de transmissão).
- `./jarvis wifi-passwords [SSID] [--limit N] [--show-clear] [--dry-run]` — lista redes Wi-Fi conhecidas ou recupera com segurança senhas autorizadas no Keychain do macOS (`security find-generic-password`).
- `./jarvis mac-specs [--dry-run]` — especificações consolidadas de hardware (versão do macOS, chip Apple Silicon / processador, contagem de núcleos, memória RAM total, armazenamento em disco e uptime).
- `./jarvis network-quality [--dry-run]` — teste nativo de capacidade e qualidade de rede do macOS (`/usr/bin/networkQuality`), medindo vazão de download, upload e responsividade (RPM).
- `./jarvis weather [cidade] [--dry-run]` — inteligência meteorológica instantânea via `wttr.in` com geolocalização automática ou consulta por cidade, retornando temperatura, sensação térmica, umidade, vento, chuva e previsão para hoje e amanhã sem chaves de API.
- `./jarvis qr "texto/url" [--output P] [--dry-run]` — gerador de QR Code com renderização ANSI direta no terminal ou gravação em arquivo PNG.
- `./jarvis crypto-stock [tickers...] [--dry-run]` — telemetria financeira em tempo real com cotações de Bitcoin, Ethereum, Solana e conversão cambial comercial Dólar (USD) e Euro (EUR) para Real (BRL).
- `./jarvis tech-brief [--limit N] [--dry-run]` — briefing executivo com as principais notícias de tecnologia e desenvolvimento em destaque no Hacker News.
- `./jarvis wiki "termo" [--lang pt|en] [--dry-run]` — assistente de consulta rápida à Wikipédia via REST API oficial, devolvendo título, contexto e resumo enciclopédico.
- `./jarvis workspace foco|reuniao|codigo|off [--dry-run]` — orquestrador de modos de trabalho do Mac, integrando ajuste de volume, reprodução ou pausa no Spotify, notificações e abertura de ferramentas de desenvolvimento.
- `./jarvis file-organize [PASTA] [--apply] [--limit N] [--dry-run]` — organização segura de arquivos soltos em subpastas categorizadas (`JARVIS_ORGANIZED/documents`, `images`, `code`, etc.); preview seguro por padrão e execução ativa com resolução de colisões apenas com a flag `--apply`.
